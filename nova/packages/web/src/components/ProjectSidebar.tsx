...
import { Card, Typography, Tree, Tag, Empty, Collapse, Descriptions, Space, List, Button } from 'antd';
import { GlobalOutlined, TeamOutlined, OrderedListOutlined, MessageOutlined, PlusOutlined } from '@ant-design/icons';
import { getProject, listThreads, type ProjectSnapshot, type ChatThread } from '../services/api';
...
export default function ProjectSidebar({
  ...
  useEffect(() => {
    if (!projectId) return;
    listThreads(projectId).then((data) => setThreads(data.threads || [])).catch(() => {});
  }, [projectId, threadListRefreshKey]);
  ...
  const collapseItems = [
    {
      ...
      children: (
        <div style={{ marginBottom: 8 }}>
          <Button
            type={currentThreadId === null ? 'primary' : 'default'}
            size="small"
            icon={<PlusOutlined />}
            block
          ...
          <List
            ...
            renderItem={(t) => (
              <List.Item
                style={{
                  cursor: 'pointer',
                  background: t.threadId === currentThreadId ? 'var(--ant-color-primary-bg)' : undefined,
                  borderRadius: 4,
                  padding: '4px 8px',
                }}
                onClick={() => onThreadSelect?.(t.threadId)}
              >
                <Text ellipsis style={{ fontSize: 12 }}>
                  {t.title || `对话 ${t.messageCount} 条`}
                </Text>
              </List.Item>
            )}
          />
  ...
  return (
    <div style={{ padding: '12px 0', height: '100%', overflow: 'auto' }}>
      <Collapse
        items={collapseItems}
        defaultActiveKey={['chats', 'info', 'world', 'characters', 'outline']}
        ghost
        size="small"
      />
    </div>
